...
class TextEmbeddingLogs(MainDB_Base):
    ...
    # Primary key, unique identifier for each log
    id: Mapped[str] = mapped_column(String, primary_key=True)

    # Original text that was embedded (this is the observation node text, semantically generated by LLM)
    observation_text: Mapped[str] = mapped_column(Text, nullable=False)

    # Embedding vector (default 3072 dimensions for gemini-embedding-001, adjust as needed)
    embedding_vector: Mapped[list[float]] = mapped_column(Vector(3072), nullable=False)

    # Metadata
    # timestamp
    created_at: Mapped[datetime] = mapped_column(DateTime, index=True, nullable=False, default=datetime.now)
    # reference to original observation if needed
    ...
    __table_args__ = (
        ...
        # Using cosine distance (NOTE: can also use 'vector_l2_ops' for L2 distance)
        Index(
